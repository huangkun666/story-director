...
- **大纲生成**：读取当前角色卡（含 depth_prompt、世界书、聊天级 scenario/system_prompt/mes_example 覆盖），按用户指定的**故事内时间线**生成完整故事大纲：分幕结构（起承转合）+ 6-8 个情节节点（带铺垫/冲突/转折/高潮/收束类型）+ 角色弧光进度 + 伏笔回收关联；
...
- **分栏工作台**：挂在聊天输入区左侧的魔法棒菜单里，打开白色大窗口；主界面只有「大纲总览」页签，时间线、参数、独立 API、故事设定与工具都收在「设置与工具」页签；窗口可拖拽、Esc 关闭；
