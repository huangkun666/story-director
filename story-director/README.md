...
- **修订快照**：生成/修订/体检/跳转游玩/清空/导入前自动留快照（最多 30 条），可从左侧工具卡一键回滚；支持导出/导入大纲 JSON；
- **世界模式**：大纲只规划世界事件表与各方计划（含触发时机），**永不规划主角行动**——主角完全自由，世界会回应；事件表由修订推进（⚡进行中/⏳待触发/🌫背景注入对话）；
- **操作级撤销**：手动编辑（节点/幕/弧光/伏笔/时间线/导入/清空/跳转）逐步入撤销栈（内存 20 步），工具栏「撤销」按钮或 Ctrl+Z 逐步回退；连续输入合并为一步；大操作回滚走持久快照；
- **调试终端**：「API 与工具」页签内嵌实时日志——LLM 调用（模型/耗时/角色卡预算占用）、检索命中、记忆指针缺口、大纲自愈事件、编辑流、生命周期；分级过滤 + 关键字搜索 + 展开详情 + 导出 JSON；
